...
def run_experiment(optimal_init_pos, number_of_trials, AirSim, AirLearning, Darp3D, Last, Hoffset, margin, RefineWPS):
    ...
    if Darp3D:
        ...
        if Last:
            ...
            asdfg = asdf.getWaypoints()
            cam_angles = asdf.getCameraAngles()

            np.savez('data.npz', asdfg=asdfg, cam_angles=cam_angles)

        init_asdfg = []

        for i in range(real_world_parameters.droneNo):
            init_asdfg.append(asdfg[i][0])

        run_airsim(real_world_parameters, init_asdfg, asdfg, cam_angles, 1)

    elif AirSim:
        run_airsim(real_world_parameters, init_posNED, WaypointsNED, 0, 0)
...
def initializeDARPGrid(randomInitPos, l, m, initialPos, megaNodes, theta, shiftX, shiftY, droneNo):
    ...
    if randomInitPos:
        ...
        initPosNED = ConvCoords(real_world_parameters.geoCoords, real_world_parameters.geoObstacles).convWGS84ToNED(initialPos)
...
if __name__ == '__main__':
    ...
    argparser.add_argument(
        '-Darp3D',
        action='store_true',
        help='Run Simulation Using darp3D')
    argparser.add_argument(
        '-Last',
        action='store_true',
        help='Use previously calculated waypoints for Darp3D.')
    argparser.add_argument(
        '-Hoffset',
        type=float,
        default = None,
        help='Modify the height of the waypoints by Hoffset')
    argparser.add_argument(
        '-margin',
        type=float,
        default = None,
        help='Modify the height margin when adding a waypoint')
    argparser.add_argument(
        '-RefineWPS',
        action='store_true',
        default = None,
        help='Refine waypoints to respect vertical offset distance.')

    args = argparser.parse_args()
    run_experiment(args.optimization, args.number_of_trials, args.AirSim, args.AirLearning, args.Darp3D, args.Last, args.Hoffset, args.margin, args.RefineWPS)
